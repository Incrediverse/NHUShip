// Copyright 2021 Xsens Technologies B.V., Inc. All Rights Reserved.
...
FString MvnRemoteControlSessionUtil::FormulateTimeCodeString( double _dTimeOffsetInSeconds )
{
    ...
    FDateTime timeWithOffset = FDateTime::Now();
    timeWithOffset += offsetSpan;

	FTimecode timeCode = FApp::GetTimecode();

    /// <CaptureStart> and <CaptureStop> messages should be the current time increased with half a second, and be rounded up to the nearest whole second.
    if ( _dTimeOffsetInSeconds > 0.0 )
    {
        if ( timeWithOffset.GetMillisecond() > 500 )
        {
            timeCode.Seconds++;
        }
    }

    return FString::Printf( TEXT( "%02d %02d %02d" ), timeCode.Hours, timeCode.Minutes, timeCode.Seconds);
}
